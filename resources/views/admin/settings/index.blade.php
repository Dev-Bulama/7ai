...
@if($tab === 'general')
<div class="card">
  <div class="form-section-title" style="font-weight:700;font-size:14px;margin-bottom:20px;">General Settings</div>
  <div class="form-grid">
  ...
  <div class="form-section-title" style="font-weight:600;font-size:13px;margin-bottom:16px;color:var(--gray-700);">Navigation CTA</div>
...
<div class="card">
  ...
  <div style="border-top:1px solid var(--gray-200);margin:20px 0;"></div>
  <div class="form-grid">
    <div class="form-group"><label class="form-label">Primary Color</label><div style="display:flex;gap:8px;align-items:center;"><input type="color" name="primary_color" value="{{ $s('primary_color','#0B4F6C') }}" style="width:48px;height:38px;padding:2px;border:1px solid var(--gray-300);border-radius:4px;cursor:pointer;"><input class="form-input" value="{{ $s('primary_color','#0B4F6C') }}" style="flex:1;" readonly></div></div>
    <div class="form-group"><label class="form-label">Secondary Color</label><div style="display:flex;gap:8px;align-items:center;"><input type="color" name="secondary_color" value="{{ $s('secondary_color','#3EE07F') }}" style="width:48px;height:38px;padding:2px;border:1px solid var(--gray-300);border-radius:4px;cursor:pointer;"><input class="form-input" value="{{ $s('secondary_color','#3EE07F') }}" style="flex:1;" readonly></div></div>
  </div>
...
<div class="card" style="margin-bottom:24px;">
  ...
  <p style="font-size:13px;color:var(--gray-500);margin-bottom:20px;">Configure your outbound email server. These settings override the server's .env mail config. Leave blank to use .env defaults.</p>
  <div class="form-grid">
    <div class="form-group"><label class="form-label">SMTP Host</label><input name="mail_host" class="form-input" value="{{ $s('mail_host') }}" placeholder="mail.7ai.africa or smtp.mailgun.org"></div>
  ...
  <div class="form-grid">
    ...
    <div class="form-group"><label class="form-label">SMTP Password <span style="font-weight:300;color:var(--gray-400);">(leave blank to keep existing)</span></label><input type="password" name="mail_password" class="form-input" autocomplete="new-password" placeholder="••••••••"></div>
  </div>
  <div class="form-grid">
    <div class="form-group"><label class="form-label">Encryption</label>
      <select name="mail_encryption" class="form-input">
        @foreach(['tls' => 'TLS (port 587)', 'ssl' => 'SSL (port 465)', '' => 'None (not recommended)'] as $val => $label)
        <option value="{{ $val }}" {{ $s('mail_encryption','tls') === $val ? 'selected' : '' }}>{{ $label }}</option>
        @endforeach
      </select>
    </div>
  ...
  <button type="submit" class="btn btn-primary" style="margin-right:8px;">Save SMTP Settings</button>
</div>

<div class="card">
  <div style="font-weight:700;font-size:14px;margin-bottom:6px;">Send Test Email</div>
  <p style="font-size:13px;color:var(--gray-500);margin-bottom:16px;">Verify your SMTP settings are working by sending a test email.</p>
  <form method="POST" action="{{ route('admin.settings.test-email') }}">
    @csrf
    <div style="display:flex;gap:12px;align-items:flex-end;">
      <div class="form-group" style="flex:1;margin-bottom:0;"><label class="form-label">Send test email to</label><input type="email" name="test_email" class="form-input" placeholder="you@example.com" required></div>
...
<div class="card">
  <div style="font-weight:700;font-size:14px;margin-bottom:16px;">Footer Social Links</div>
  <p style="font-size:13px;color:var(--gray-500);margin-bottom:16px;">These appear as icon links in the site footer. Use the Social Media URLs above for page-level sharing.</p>
...
<div class="card">
  ...
  <p style="font-size:13px;color:var(--gray-500);margin-bottom:20px;">Scripts are injected into every page. Use with care — invalid scripts can break the site.</p>
...
<div class="card">
  ...
  <div style="display:flex;flex-direction:column;gap:16px;margin-bottom:20px;">
    ...
    <label class="form-check">
      ...
      <span style="display:block;font-size:12px;color:var(--gray-400);margin-top:2px;">Uncheck to disable the public registration page.</span>
    </label>
    <label class="form-check">
